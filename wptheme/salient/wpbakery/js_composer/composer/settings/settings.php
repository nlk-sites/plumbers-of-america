...
* @param $callback - function to build section header.
    ...
    protected function getExcluded() {
        return array('attachment', 'revision', 'nav_menu_item', 'mediapage', 'home_slider');
    }
...
EOF;
        ...
        <form action="options.php" method="post" id="vc-settings-<?php echo $tab ?>" class="vc-settings-tab-content<?php echo ($this->active_tab == $tab ? ' vc-settings-tab-content-active' : '').$css ?>"<?php echo apply_filters('vc_setting-tab-form-'.$tab, '') ?>>
            ...
            <?php if($tab === 'updater'): ?>
            <?php
?>          <span class="vc-updater-spinner-wrapper" style="display: none;" id="vc-updater-spinner"><img src="<?php echo get_site_url() ?>/wp-admin/images/wpspin_light.gif"  /></span>
            <?php endif; ?>
        </form>
        <?php endif; ?>
