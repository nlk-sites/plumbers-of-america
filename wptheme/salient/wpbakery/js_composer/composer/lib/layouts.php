...
class WPBakeryVisualComposerElementButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        ...
        $output .= '<li data-element="' . $this->base . '" class="wpb-layout-element-button'.$category_css_classes.$class_out.'"'.$data.'><a id="' . $this->base . '" class="dropable_el clickable_action'.$class.'" href="#">' . $this->getIcon() . htmlspecialchars(__(stripslashes($this->params["name"]), "js_composer")) .$description.'</a></li>';
        return $output;
    }
}
    ...
    public function output($post = null) {
        ...
        <div class="metabox-composer-content">
					<div id="wpb-convert-message">
					   <div class="messagebox_text"><p>'.__('Your page layout was either created with a previous Visual Composer version or you have elements/text located outside of VC rows (which could have occured if you switched into the classic editor view to make changes). Visual Composer requires all content be contained within rows to render correctly - before converting your layout to the new version, make sure to <a target="_blank" href="http://kb.wpbakery.com/index.php?title=Update_Visual_Composer_from_3.4_to_3.5">read this page</a>.', "js_composer").'</p>
