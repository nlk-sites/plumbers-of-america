...
(function ($) {
    ...
    window.VcRowView = vc.shortcode_view.extend({
        ...
        changeShortcodeParams:function (model) {
          ...
          if(!_.isEmpty(params.background_color)) {
              $('<span class="vc_row_color" style="background-color: ' + params.background_color + '" title="' + i18nLocale.row_background_color + '"></span>').insertAfter($column_edit);
          }
    ...
    window.VcAccordionView = vc.shortcode_view.extend({
        ...
        addTab:function (e) {
            ...
            e.preventDefault();
            vc.shortcodes.create({shortcode:'toggle', params:{title:window.i18nLocale.section}, parent_id:this.model.id});
        },
        ...
        changedContent:function (view) {
            ...
            if (view.model.get('cloned') === true) {
                ...
                var cloned_from = view.model.get('cloned_from'),
                    $after_tab = $('[href=#tab-' + cloned_from.params.id + ']', this.$content).parent(),
                    $new_tab = $("<li><a href='#tab-" + params.id + "'>" + params.title + "</a></li>").insertAfter($after_tab);
    ...
    window.VcTabView = window.VcColumnView.extend({
        ...
        cloneModel:function (model, parent_id, save_order) {
            var shortcodes_to_resort = [],
                new_order = _.isBoolean(save_order) && save_order === true ? model.get('order') : parseFloat(model.get('order')) + vc.clone_index,
                new_params = _.extend({}, model.get('params'));
            if (model.get('shortcode') === 'tab') _.extend(new_params, {id:+new Date() + '-' + this.$tabs.find('[data-element_type=tab]').length + '-' + Math.floor(Math.random() * 11)});
    ...
    window.VcTabView35 = window.VcColumnView.extend({
        ...
        deleteShortcode:function (e) {
            ...
            this.model.destroy();
          var params = this.model.get('params');
            this.$tabs.tabs("remove", $('[href=#tab-' + params.id + ']').parent().index());
        },
        cloneModel:function (model, parent_id, save_order) {
            var shortcodes_to_resort = [],
                new_order = _.isBoolean(save_order) && save_order === true ? model.get('order') : parseFloat(model.get('order')) + vc.clone_index,
